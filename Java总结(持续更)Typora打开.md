...
3. 如果 hash 冲突，两种解决方案：链表 or 红黑树；如果是链表，把新元素追加到队尾，当链表的长度大于等于 8 时,转4；如果是红黑树，调用红黑树新增的方法； 
4. 数组长度小于64(MIN_TREEIFY_CAPACITY)，则会扩容数组，否则树化
5. 根据临时变量判断有没有内容相同的元素，临时变量不为null，再根据 onlyIfAbsent 判断是否需要覆盖； 
6. 判断是否需要扩容，需要扩容进行扩容，结束。  
...
**对⻬填充部分 ** **仅仅起到占位符的作⽤**。HotSpot虚拟机的自动内存管理系统要求对象起始地址必须是8字节的整数倍，即对象大小必须是8字节的整数倍 
...
- 对比
  ...
  对⽐ 垃圾回收分析：⽅式1当垃圾回收移动对象时，reference中存储的地址是稳定的地址，不需 要修改，仅需要修改对象句柄的地址；⽅式 2 垃圾回收时需要修改reference中存储的地址。 
